-- Exams: random draw of N questions per attempt (null = use all)
alter table exams add column if not exists draw_count integer;

-- Banks: logo + per-bank account custom fields + supported payment channels
alter table banks add column if not exists logo_path text;
alter table banks add column if not exists account_fields jsonb not null default '[]'::jsonb; -- [{key,label}]
alter table banks add column if not exists channels jsonb not null default '[]'::jsonb;       -- ["PromptPay","MoMo"]

-- Bank Accounts module: submitted from the app (Submit New Account) or created by admins
