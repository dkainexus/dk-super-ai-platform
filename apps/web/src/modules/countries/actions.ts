...
export async function updateCountry(formData: FormData): Promise<void> {
  ...
  const { error } = await db().from("countries").update({
      ...
      usdt_markup_pct: parseFloat(String(formData.get("usdt_markup_pct") ?? "0")) || 0,
      usdt_markup_flat: parseFloat(String(formData.get("usdt_markup_flat") ?? "0")) || 0,
      usdt_address_trc20: String(formData.get("usdt_address_trc20") ?? "").trim() || null,
